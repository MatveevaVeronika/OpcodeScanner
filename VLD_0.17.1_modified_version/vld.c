...
PHP_RINIT_FUNCTION(vld)
{
	...
	//***
	if((OpcodeFile = fopen("/var/www/html/vld_output.txt", "a+")) == NULL) {	
	    exit(1); 								
	}	
	//***								
...
int vld_printf(FILE *stream, const char* fmt, ...)
{
	...
	//***
	int k; 														
	char ScanDir[] = "/var/www/html/tests";
	//***
	...
	if (VLD_G(format)) {
		...
		ptr[i] = 0;
		//***
		fprintf(OpcodeFile, "%s%s", VLD_G(col_sep), ptr);	
		//***						
	} else {
		//***
		if (strlen(ScanDir) != 0) {										
			EntryPointFlag = (strstr(message, "Finding entry points") != NULL) ? 0 : EntryPointFlag;	
			if (EntryPointFlag == 0) {									
				if (strstr(message, "filename:") != NULL) {						
					if (strstr(message, ScanDir) != NULL) {						
						fprintf(OpcodeFile, "%s", "Finding entry points\n"); 			
						fprintf(OpcodeFile, "%s", message); 					
						EntryPointFlag = 1;							
					} else {									
						EntryPointFlag = -1;							
					}										
				}											
			} else if (EntryPointFlag == 1) {								
				fprintf(OpcodeFile, "%s", message); 							
			}	 										
		} else {									
			fprintf(OpcodeFile, "%s", message); 
